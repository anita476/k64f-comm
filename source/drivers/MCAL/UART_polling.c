...
static UARTConfig_t uart_pin_map[UART_ALTS]={
    {0,PB,PORTNUM2PIN(PB, 17),PORTNUM2PIN(PB, 16),PORT_mAlt3},
    {1,PE,PORTNUM2PIN(PE, 0),PORTNUM2PIN(PE, 1),PORT_mAlt3},
    {2,PD,PORTNUM2PIN(PD, 3),PORTNUM2PIN(PD, 2),PORT_mAlt3},
    {3,PE,PORTNUM2PIN(PE, 4),PORTNUM2PIN(PE, 5),PORT_mAlt3},
...
uint32_t const uart_sim_masks[UART_COUNT] = {SIM_SCGC4_UART0_MASK,SIM_SCGC4_UART1_MASK,SIM_SCGC4_UART2_MASK,SIM_SCGC4_UART3_MASK,SIM_SCGC1_UART4_MASK};
uint32_t const uart_nvim_ints[UART_COUNT] = {UART0_RX_TX_IRQn,UART1_RX_TX_IRQn,UART2_RX_TX_IRQn,UART3_RX_TX_IRQn,UART4_RX_TX_IRQn};
...
uint32_t UART_polling_drv_instance_init (pin_t RX_pin, pin_t TX_pin){
    ...
    if(id == INVALID_UART) { return INVALID_UART;}
    ...
    port_ptrs[uart_pin_map[uart_num_index[id]].uart_port]->PCR[PIN2NUM(uart_pin_map[uart_num_index[id]].tx_pin)] |= PORT_PCR_MUX(uart_pin_map[uart_num_index[id]].alt); 
    port_ptrs[uart_pin_map[uart_num_index[id]].uart_port]->PCR[PIN2NUM(uart_pin_map[uart_num_index[id]].tx_pin)] |= PORT_PCR_IRQC(PORT_eDisabled);
    
    port_ptrs[uart_pin_map[uart_num_index[id]].uart_port]->PCR[PIN2NUM(uart_pin_map[uart_num_index[id]].rx_pin)] = 0x0; 
    port_ptrs[uart_pin_map[uart_num_index[id]].uart_port]->PCR[PIN2NUM(uart_pin_map[uart_num_index[id]].rx_pin)] |= PORT_PCR_MUX(uart_pin_map[uart_num_index[id]].alt); 
    port_ptrs[uart_pin_map[uart_num_index[id]].uart_port]->PCR[PIN2NUM(uart_pin_map[uart_num_index[id]].rx_pin)] |= PORT_PCR_IRQC(PORT_eDisabled);
